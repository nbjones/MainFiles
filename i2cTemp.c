...
//		}
		case vtI2CMsgTypeTempRead1: {
			if (currentState == fsmStateTempRead1) {
				...
				int ADCPacket[3] = {temperature, 0, 0};
				...
				if (lcdData != NULL) {
					timerTest++;
					if(timerTest >= 100) {
						SendLCDPrintMsg(lcdData,strnlen(lcdBuffer,vtLCDMaxLen),lcdBuffer,portMAX_DELAY);
						timerTest = 0;
					}
					//if (SendLCDADC(lcdData,3,ADCPacket,portMAX_DELAY) != pdTRUE) {
					//	VT_HANDLE_FATAL_ERROR(0);
					//}
				}
			} else {
